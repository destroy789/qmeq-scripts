...
def stab_calc(system, bfield, vlst, vglst, dV=0.0001):
    ...
    for j1 in range(vgpnt):
        ...
        for j2 in range(vpnt):
            vbiasL = vlst[j2]/2
            vbiasR = -vlst[j2]/2
            xxx = - Cg2/Csigma * vglst[j1] - Cg1/Csigma * vglst[j1] #- Cd/Csigma * vlst[j2]
            yyy = - Cgd2/Csigmad * vglst[j1] - Cgd1/Csigmad * vglst[j1] #- Cdd/Csigmad * vlst[j2]
            ...
            system.change(hsingle={(0, 0):-0.5*E_c1 - 3 * E_c1 + q2 * E_m + xxx,
                          (1, 1): -0.5*E_c1 - 2 * E_c1 + q2 * E_m + xxx, 
                          (2, 2): -0.5*E_c1 - 1 * E_c1 + q2 * E_m + xxx,
                          (3, 3): -0.5*E_c1 + 0 * E_c1 + q2 * E_m + xxx,
                          (4, 4): -0.5*E_c1 + 1 * E_c1 + q2 * E_m + xxx,
                          (5, 5): -0.5*E_c1 + 2 * E_c1 + q2 * E_m + xxx,
                          (6, 6): -0.5*E_c2 - 1 * E_c2 + q1 * E_m + yyy,
                          (7, 7): -0.5*E_c2 + 0 * E_c2 + q1 * E_m + yyy,
                          (0, 6): -omega,
                          (1, 6): -omega,
                          (2, 6): -omega,
                          (3, 6): -omega,
            ...
            system.change(mulst={0: vbiasL, 1: vbiasR, 2: vbiasL+10, 3: vbiasR+10})
            system.solve(qdq=False)
            stab[j1, j2] = abs(system.current[0] + system.current[2])
            #
...
vglst = np.linspace(-200, 100, vgpnt)
